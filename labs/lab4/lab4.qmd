...
To determine the notes' error range, we can once again take a look at the PSC and ARR values specified at `TIM16`, which had a PWM and generated a square wave to create the pitch of each note. As its `PSC` was determined to be 20, we know that its $f_{CNT}$, or counter clock frequency, must be $80,000,000 / (20+1) = 3809523.8$. However, these values are truncated as a result of C convention, which effectively means that the utilized counter clock frequency is $f_{CNT} = 3809523$. We can determine some `ARR` values for the minimum and maximum notes playable (220 Hz and 1000 Hz): $ARR_{min} = \frac{3809523}{220} - 1 = 17315$, $ARR_{max} = \frac{3809523}{1000} - 1 = 3808$. Via these values,we can see how the `PSC` and `ARR` values come to output a real frequency. $f_{min} = \frac{3809523.8}{17315 + 1} = 220.00021945$ and $f_{max} = \frac{3809523.8}{3808 + 1} = 1000.13751641$. By normal error equation, we get that $ERR_{min} = \frac{220.00021945 - 220}{220} = 0.0000009975$ and $ERR_{max} = \frac{1000.13751641 - 1000}{1000} = 0.00013751641$, both of which are within 1% error bounds.
